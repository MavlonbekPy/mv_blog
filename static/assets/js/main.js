...
function animFly(items, options) {
    ...
    TweenMax.staggerTo(items, options.duration, {delay: options.delay, opacity: 1}, options.repeatDelay)
}
...
if (staggerAnims) {
    staggerAnims.forEach(function (anim) {
        ...
        var animState = false;
        if (animScroll == 'false') {
            animFly(animItems, {duration: .85, delay: .3, y: yAnim, repeatDelay: .2})
            animState = true;
        } else {
            animWatcher.enterViewport(function () {
                if (!animState) {
                    animFly(animItems, {duration: .85, delay: 0, y: yAnim, repeatDelay: .2})
                    animState = true;
                }
            })
        }
